...
def check_credentials():
    try:
        ...
        if "ExpiredToken" in msg or "InvalidClientTokenId" in msg:
            problems.append(
                "Credentials expired — paste a fresh block from the AWS "
                "portal, or re-run 'aws configure'"
            )
        else:
            problems.append(
                "Set credentials: paste your SSO block, or run 'aws configure'"
            )
        return False
    return True
...
def check_region():
    ...
    if region == "us-east-1":
        ...
        print(
            f"{WARN} AWS_DEFAULT_REGION not set — run: "
...
def check_bedrock():
    try:
        ...
        if "ThrottlingException" in msg:
            print(f"{WARN} Bedrock model call throttled — daily token quota reached")
            print(
                "       Credentials and model access are valid; try again "
